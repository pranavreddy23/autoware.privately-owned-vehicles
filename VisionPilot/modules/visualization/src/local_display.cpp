    ...
    bool LocalDisplay::render_frame(const cv::Mat& display_frame)
    {
        ...
        if (!aux_frame_.empty()) {
            if (aux_frame_.rows != display_frame.rows && aux_frame_.rows > 0) {
                const double s = static_cast<double>(display_frame.rows) /
                                 static_cast<double>(aux_frame_.rows);
        ...
        if (!occ_show.empty()) {
            ...
            const bool size_changed =
                display_frame.cols != last_main_w_ ||
                display_frame.rows != last_main_h_ ||
                occ_show.cols != last_aux_w_ ||
                occ_show.rows != last_aux_h_;

            if (!layout_done_ || size_changed) {
                cv::moveWindow("VisionPilot", kOriginX, kOriginY);
                cv::moveWindow("Occupancy",
                               kOriginX + display_frame.cols + kGap,
                               kOriginY);
                layout_done_ = true;
