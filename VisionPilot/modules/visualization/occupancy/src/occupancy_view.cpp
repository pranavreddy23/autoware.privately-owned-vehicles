...
static OccBasis make_occ_basis(int pw, int ph) {
    ...
    b.f0 = fx / fl; b.f1 = fy / fl; b.f2 = fz / fl;
    ...
    rx = b.f1; ry = -b.f0; rz = 0.f;
    float rl = std::sqrt(rx * rx + ry * ry + rz * rz) + 1e-6f;
    b.r0 = rx / rl; b.r1 = ry / rl; b.r2 = rz / rl;
...
static bool project_xyz(const OccBasis& b, float x, float y, float z,
                        cv::Point& out, float* depth = nullptr) {
...
static void extrude_box_3d(
    cv::Mat& layer,
    const OccBasis& basis,
    float x0, float y0, float x1, float y1,
    float z0, float z1,
    cv::Scalar paint, float fade, float top_boost = 1.10f)
{
    if (z1 <= z0 + 0.05f) return;
    auto scale_bgr = [](cv::Scalar c, float s) -> cv::Scalar {
        return cv::Scalar(std::min(255.0, c[0] * s),
                          std::min(255.0, c[1] * s),
                          std::min(255.0, c[2] * s));
    };
    ...
    struct Face { float depth; int kind; int i; }; // kind 0=wall, 1=top
    std::array<Face, 5> faces;
    for (int i = 0; i < 4; ++i) {
        const int j = (i + 1) % 4;
        faces[static_cast<size_t>(i)] = {
            0.25f * (fd[i] + fd[j] + td[i] + td[j]), 0, i};
    }
    ...
    for (const auto& f : faces) {
        if (f.kind == 1) {
            ...
            cv::polylines(layer, poly, true,
                          scale_bgr(cv::Scalar(255, 255, 255), 0.35f * fade), 1, cv::LINE_AA);
            cv::line(layer, top[0], top[1],
                     scale_bgr(cv::Scalar(255, 255, 255), 0.55f * fade), 1, cv::LINE_AA);
        } else {
            const int i = f.i;
            const int j = (i + 1) % 4;
...
static void extrude_box_yaw(
    cv::Mat& layer,
    const OccBasis& basis,
    float x_fwd, float y_lat,
    float half_l, float half_w,
    float z0, float z1,
    float yaw,
    const cv::Scalar& paint,
    float fade,
    float top_boost = 1.12f)
{
    ...
    float yaw = 0.f)
{
    ...
    const cv::Scalar paint = is_cipo ? cv::Scalar(40, 40, 220)
                                     : vehicle_paint(class_id, is_truck);
    ...
    // Ground shadow
    {
        ...
        if (project_xyz(basis, x_fwd, y_lat, 0.f, sc)) {
            ...
            cv::ellipse(layer, sc + cv::Point(1, 2),
                        cv::Size(std::max(4, static_cast<int>(width_m * 5.0f * d)),
                                 std::max(2, static_cast<int>(length_m * 2.2f * d))),
                        yaw * 180.f / static_cast<float>(CV_PI), 0, 360,
                        cv::Scalar(14, 12, 10), -1, cv::LINE_AA);
        }
    }
    ...
    if (std::abs(yaw) > 1e-3f) {
        ...
        if (!is_truck) {
            extrude_box_yaw(layer, basis, x_fwd, y_lat, half_l, half_w,
                            0.f, 1.45f, yaw, paint, fade, is_cipo ? 1.05f : 1.12f);
        } else {
            extrude_box_yaw(layer, basis, x_fwd, y_lat, half_l, half_w,
                            0.f, 3.0f, yaw, paint, fade, 1.05f);
        }
        return;
    }

    if (!is_truck) {
        extrude_box_3d(layer, basis,
                       x_fwd - half_l, y_lat - half_w,
                       x_fwd + half_l, y_lat + half_w,
                       0.f, 1.45f, paint, fade, is_cipo ? 1.05f : 1.12f);
    } else {
        const float split = x_fwd + half_l * 0.35f;
        extrude_box_3d(layer, basis,
                       x_fwd - half_l, y_lat - half_w,
                       split, y_lat + half_w,
                       0.f, 3.2f, scale_bgr(paint, is_cipo ? 1.0f : 0.90f), fade, 1.05f);
        extrude_box_3d(layer, basis,
                       split - half_l * 0.02f, y_lat - half_w * 0.88f,
                       x_fwd + half_l, y_lat + half_w * 0.88f,
                       0.f, 2.5f, scale_bgr(paint, is_cipo ? 1.05f : 1.05f), fade, 1.12f);
    }
}
...
cv::Mat render(const Scene& scene) {
    ...
    // Ground plane
    {
        ...
        ground.reserve(4);
        for (auto [x, y] : {std::pair<float,float>{0.f, kYMax}, {0.f, -kYMax},
    ...
    for (float x = 0.f; x <= kXMax + 0.01f; x += 5.f) {
        ...
        if (x > 0.f && static_cast<int>(x) % 20 == 0) {
            ...
            if (project_xyz(basis, x, -kYMax + 0.6f, 0.f, p))
                cv::putText(panel, lbl, p + cv::Point(3, -3),
    ...
    if (scene.path_valid) {
        ...
        for (float x = x0; x <= x1 + 0.01f; x += 1.0f) {
            ...
            lp.push_back(pl); rp.push_back(pr); mid.push_back(pm);
        }
        if (lp.size() >= 2) {
    ...
    if (scene.lane_world.size() >= 2) {
        ...
        for (const auto& w : scene.lane_world) {
            ...
            cv::Point p;
            if (project_xyz(basis, w.x, w.y - y_shift, 0.03f, p))
                lane_px.push_back(p);
        }
        if (lane_px.size() >= 2)
            cv::polylines(panel, lane_px, false, kLane, 2, cv::LINE_AA);
    }
    ...
    const float focus_x = (scene.cipo_valid && scene.cipo_distance_m > 0.5f)
                              ? scene.cipo_distance_m
                              : ((scene.ad_cipo_only && scene.ad_distance_m > 0.5f)
                                     ? scene.ad_distance_m : -1.f);
    ...
    for (const auto& vox : voxels) {
        draw_extruded_vehicle(
            panel, basis,
            vox.x, vox.y,
            vox.half_l * 2.f, vox.half_w * 2.f,
            vox.is_truck,
            vox.class_id,
            depth_fade(vox.x, vox.y),
            vox.is_cipo);
    }
    ...
    if (cipo_car != nullptr) {
        ...
        if (project_xyz(basis, cipo_car->x, cipo_car->y, z_lbl, p)) {
            ...
            cv::rectangle(panel,
                          origin + cv::Point(-4, -ts.height - 2),
                          origin + cv::Point(ts.width + 4, 4),
                          cv::Scalar(20, 20, 20), -1);
            cv::putText(panel, lbl, origin,
                        cv::FONT_HERSHEY_SIMPLEX, 0.48, cv::Scalar(80, 80, 255), 1, cv::LINE_AA);
        }
        ...
        if (project_xyz(basis, scene.ad_distance_m, yw, 0.4f, p))
            cv::arrowedLine(panel, p + cv::Point(0, 16), p, cv::Scalar(40, 40, 220), 2, cv::LINE_AA, 0, 0.35);
    }
    ...
    if (std::abs(g_cte_s) > 0.35f) {
        ...
        cv::putText(panel, cte_lbl, cv::Point(12, 48),
                    cv::FONT_HERSHEY_SIMPLEX, 0.40, cv::Scalar(230, 230, 230), 1, cv::LINE_AA);
    }

    cv::rectangle(panel, cv::Point(0, 0), cv::Point(pw - 1, ph - 1),
                  cv::Scalar(70, 85, 50), 1, cv::LINE_AA);
    ...
    cv::putText(panel, "OCCUPANCY", cv::Point(12, 18),
    ...
    cv::putText(panel, range_lbl, cv::Point(pw - ts.width - 12, 18),
                cv::FONT_HERSHEY_SIMPLEX, 0.38, cv::Scalar(150, 160, 130), 1, cv::LINE_AA);

    // Interaction hint
    cv::putText(panel, "L-drag orbit  R-drag pan  wheel zoom  R reset",
                cv::Point(12, ph - 12),
