...
Scene make_scene(const visionpilot::models::InferenceFrameResult& r,
                 const Plan& /*plan*/,
                 const cv::Mat& H_resized)
{
    ...
    if (r.auto_steer.valid && !s.H_px2world.empty()) {
        ...
        for (int i = 0; i < kSteerN; ++i) {
            if (r.auto_steer.h_vector[static_cast<size_t>(i)] < 0.5f)
                continue;
            const float u = r.auto_steer.xp[static_cast<size_t>(i)] *
                            static_cast<float>(kNetW);
            const float v = static_cast<float>(i) *
                            (static_cast<float>(kNetH - 1) /
                             static_cast<float>(kSteerN - 1));
