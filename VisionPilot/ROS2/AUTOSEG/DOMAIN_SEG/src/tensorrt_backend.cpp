  ...
  const std::string & model_path, const std::string & precision, int gpu_id)
{
  ...
  // For domain segmentation, output could be [N, 1, H, W] or [N, H, W]
  if (output_dims.nbDims == 4) {
    ...
    throw std::runtime_error("Unexpected output dimensions for domain segmentation model");
  }
  ...
  const std::string & onnx_path, const std::string & precision)
{
  ...
  for (int i = 0; i < network->getNbInputs(); ++i) {
    ...
    // Assuming typical domain segmentation input: [batch, channels, height, width]
...
{
  // Same preprocessing as ONNX Runtime backend and other models
...
void TensorRTBackend::getDomainMask(cv::Mat & domain_mask, const cv::Size & output_size) const
{
  // Convert output buffer to OpenCV Mat (raw logits, single channel float)
  cv::Mat raw_domain(model_output_height_, model_output_width_, CV_32FC1, 
