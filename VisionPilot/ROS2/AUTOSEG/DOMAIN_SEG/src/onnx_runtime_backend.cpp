...
: env_(ORT_LOGGING_LEVEL_WARNING, "domain_seg"), memory_info_(Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault))
{
...
void OnnxRuntimeBackend::initializeOrtSession(const std::string& model_path, const std::string& precision, int gpu_id)
{
  ...
  if (precision == "cuda") {
     try {
        ...
        RCLCPP_INFO(rclcpp::get_logger("domain_seg"), "Using CUDA Execution Provider on GPU %d.", gpu_id);
     } catch (const Ort::Exception& e) {
        RCLCPP_WARN(rclcpp::get_logger("domain_seg"), "CUDA Execution Provider is not available. Falling back to CPU. Error: %s", e.what());
     }
  } else {
    RCLCPP_INFO(rclcpp::get_logger("domain_seg"), "Using default CPU Execution Provider.");
  }
...
void OnnxRuntimeBackend::preprocess(const cv::Mat & input_image, std::vector<float> & output_tensor, std::vector<int64_t>& input_dims)
{
    ...
    // Same preprocessing as other models - ImageNet normalization
...
bool OnnxRuntimeBackend::doInference(const cv::Mat & input_image)
{
    ...
    try {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("domain_seg"), "ONNX Runtime inference failed: %s", e.what());
        return false;
    }
...
void OnnxRuntimeBackend::getDomainMask(cv::Mat & domain_mask, const cv::Size & output_size) const
{
    ...
    int height, width;
    // Handle different output shapes for binary segmentation: [N, H, W] or [N, 1, H, W]
    if (output_dims.size() == 4) {
        ...
        throw std::runtime_error("Unexpected output dimensions for binary domain segmentation model");
    }

    // Create domain mask from model output (raw logits, single channel float)
    cv::Mat raw_domain(height, width, CV_32FC1, const_cast<float*>(output_data));
    
    // Apply same post-processing as Python: threshold at 0 (output[output <= 0] = 0.0, output[output > 0] = 1.0)
    cv::Mat processed_domain;
    cv::threshold(raw_domain, processed_domain, 0.0, 1.0, cv::THRESH_BINARY);
    
    // Resize to desired output size
    cv::resize(processed_domain, domain_mask, output_size, 0, 0, cv::INTER_LINEAR);
}
